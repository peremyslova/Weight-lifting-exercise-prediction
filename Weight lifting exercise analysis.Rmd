...
```{r cleaning near zero variance predictors}
...
And we're quickly making sure that there are any NAs left in the data:
...
```{r data normalization for test, echo=FALSE}
...
Given the large number of potential predictors (*52*), we need to choose the key ones, i.e. the ones that have bigger impact on the outcome. We will use principal component analysis for this purpose:
...
The final model selection shows 4.42% estimate for out of sample error, which is quite good.
...
One of the PCA downsides is the lack of interpretability, so we aren't going to visualize the result decision tree. Next time we're going to try using another model to see how it compares with PCA+RF approach.
